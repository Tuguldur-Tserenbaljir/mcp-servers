...
class DockerHandlers:
    ...
    async def handle_create_container(arguments: Dict[str, Any]) -> List[TextContent]:
        try:
            ...
            # Handle ports
            ports_raw = arguments.get("ports", {})
            if isinstance(ports_raw, str):
                try:
                    ports = json.loads(ports_raw)
                except json.JSONDecodeError:
                    raise ValueError("Invalid JSON format for ports")
            else:
                ports = ports_raw
            ...
            if isinstance(env_raw, str):
                try:
                    ...
                    raise ValueError("Invalid JSON format for environment")
            else:
                environment = env_raw

            # Handle volumes
            volumes_raw = arguments.get("volumes", {})
            ...
            result = await asyncio.wait_for(pull_and_run(), timeout=DockerHandlers.TIMEOUT_AMOUNT)
            return [result]
